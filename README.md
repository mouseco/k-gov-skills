...
| 교통비 증빙 수집 | `transport-receipt-collector` | 출장·여비 정산용 하이패스·SRT·KTX/Korail 영수증을 PDF/PNG/JSON 산출물로 정리 | provider별 로컬 계정 정보 필요. KTX/Korail은 `ktx-booking` 스킬 설치 후 그 helper를 사용하는 로컬 connector 필요 | [교통비 증빙 수집 가이드](docs/features/transport-receipt-collector.md) |
