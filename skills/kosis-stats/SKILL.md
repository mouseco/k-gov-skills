---
name: kosis-stats
description: 국가데이터처가 운영하는 KOSIS(국가통계포털, kosis.kr) Open API로 한국 공식 통계표를 검색하고 메타데이터·데이터·대용량 자료를 조회한다. Use when the user asks for 한국 공식 통계 수치 조회, 특히 공공기관·공무원 보고서/사업계획/정책근거에 자주 쓰는 인구, 고령화, 청년, 가구, 고용, 물가, 지역경제, 교육, 복지 지표 탐색과 KOSIS table id 확인이 필요할 때 사용한다. Not for visualization or broad policy analysis.
...
### 0. Use public-sector indicator presets when the user asks for policy or public-sector evidence

공공기관·공무원 보고서, 사업계획서, 정책 배경자료, 지역 현황 분석처럼 "무슨 지표를 봐야 하는지"부터 정해야 하는 요청이면 먼저 [references/public-sector-indicator-presets.md](references/public-sector-indicator-presets.md)를 읽는다.

그 reference는 자주 쓰는 지표 묶음, 추천 검색어, 대표 KOSIS table id 후보, 보고서 문장화 기준을 담고 있다. 프리셋으로 후보를 좁힌 뒤 이 스킬의 `search -> meta -> data` 흐름을 적용한다.
