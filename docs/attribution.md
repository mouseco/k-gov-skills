...
- 확인된 설치 버전: `1.0.2`
- 기준 원본 위치: 설치 환경의 OpenClaw/ClawHub 스킬 디렉터리
- k-gov 저장소 반영 성격: 공공기관 보고서 근거조사용 OpenClaw Edition

### 주요 수정 방향

- 특정 사용자·로컬 환경 전용 검색 스크립트와 절대 저장 경로 제거
- 공개 저장소 사용자가 자기 작업 경로에서 쓸 수 있도록 상대 경로 중심으로 재작성
...
- 확인된 원본 위치: `NomaDamas/k-skill` 저장소의 `hwp/`
...
- 확인된 원본 위치: `NomaDamas/k-skill` 저장소의 `korean-law-search/`
...
- 성격: mouseco가 나라장터 입찰공고정보 Open API의 공개 명세를 바탕으로 작성한 조회 전용 스킬과 Python 표준 라이브러리 클라이언트
- 외부 서비스: 공공데이터포털, 조달청 나라장터 입찰공고정보서비스
- 권리 경계: 스킬 문서와 자체 스크립트는 저장소 라이선스를 따르며, API 명세·응답 데이터·서비스명은 각 제공기관의 이용 조건을 따른다.

## public-data-finder

- 성격: mouseco가 공공데이터포털 목록개방현황 Open API의 공개 명세를 바탕으로 작성한 데이터셋 탐색 스킬과 Python 표준 라이브러리 클라이언트
- 외부 서비스: 공공데이터포털 공공데이터활용지원센터 목록개방현황 API
- 권리 경계: 스킬 문서와 자체 스크립트는 저장소 라이선스를 따르며, API 명세·응답 데이터·서비스명은 공공데이터포털 이용 조건을 따른다.

## national-assembly-tracker

- 성격: mouseco가 열린국회정보 Open API의 공개 명세를 바탕으로 작성한 법률안 추적 스킬과 Python 표준 라이브러리 클라이언트
- 외부 서비스: 열린국회정보, 국회 의안정보시스템과 회의록·입법예고 공식 시스템
- 권리 경계: 스킬 문서와 자체 스크립트는 저장소 라이선스를 따르며, API 명세·응답 데이터·서비스명은 국회 공식 서비스의 이용 조건을 따른다.

## trip-expense-hwp

- 성격: mouseco가 공공기관 출장 정산 흐름에 맞춰 작성한 HWP 패키징·검증 스킬
- 연계 자산: `transport-receipt-collector`, `read-hwp`, `rhwp-edit`, 공개 배포용 HWP 샘플
- 권리 경계: 스킬 문서는 저장소 라이선스를 따른다. 연계 스킬·도구는 각자의 라이선스를 따르며, 사용자가 제공하는 기관 양식과 영수증의 공개·이용 권한은 사용자에게 있다.

## mouseco 작성 스킬

아래 스킬은 이 저장소의 공공문서·행정업무 자동화 목적에 맞춰 mouseco가 작성·정리한 스킬이다.
...
단, 각 스킬이 참조하는 공개 프로젝트, 표준, 문서 포맷, 오픈소스 도구, 공공 API와 응답 데이터의 권리는 각 원 출처에 있다.
